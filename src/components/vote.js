...
export default class MyVotes extends Component {
    ...
    votesListAPI() {
        axios.post(`${process.env.REACT_APP_URL}/api/users/voting_list`, { 'user_id': this.loginData.id, 'api_key': this.loginData.api_key },{headers}).then((res) => {
            ...
            if (this.codeDataVotes === true) {
                this.setState({
                    listVotes: res.data.recdata
                });
            }
            else if (this.codeDataVotes === false) {
                this.setState({
                    listVotes: []
                })
            }
    ...
    votingDetail(id) {
        setTimeout(() => {
            window.location.hash = '/votingDetail/' + id;
    ...
    render() {
        const codeDataVotes1 = this.codeDataVotes
                                                ...
                                                <div className="central-meta">
                                                    <div className="frnds">
                                                        <ul className="nav nav-tabs col-md-6">
                                                            <li className="nav-item"><a className="active" href="#frends" data-toggle="tab">My Votes</a> </li>
                                                            {/* <li className="nav-item"><a className="" href="#frends-req" data-toggle="tab">All Group</a></li> */}
                                                        </ul>

                                                        <div className="col-md-6 text-right">
                                                            <Link to="/voteAdd" type="submit" class="btn btn-sm btn-primary">Add Vote</Link>
                                                        </div>

                                                        <div className="tab-content">
                                                            <div className="tab-pane active fade show" id="frends" style={{ minHeight: '314px' }}>
                                                                <ul className="nearby-contct">
                                                                    {codeDataVotes1 ? '' : <div className="text-center">Nothing More To Load</div>}

                                                                    {this.state.listVotes.map(item => (
                                                                        <li>
                                                                            <div className="nearly-pepls">
                                                                                ...
                                                                                <div className="pepl-info">
                                                                                    <h4 className="voting_heading">
                                                                                        {/* <Link to={`/groupdetail/${item.id}`} onClick={this.groupDetail.bind(this, item.id)}
                                                                                            title=""> */}
                                                                                        {item.title}&nbsp;&nbsp;
                                                                                        {/* </Link> */}
                                                                                        <span>
                                                                                            <Link to={`/votingDetail/${item.id}`}>
                                                                                                <i title="Detail" className="fa fa-info-circle" onClick={this.votingDetail.bind(this, item.id)}></i></Link>&nbsp;&nbsp;
                                                                                        <i title="delete" className="fa fa-trash" onClick={this.votingDelete.bind(this, item.id)}></i>
                                                                                        </span>
                                                                                    </h4>
                                                                                    <hr />
                                                                                    {item.options.map((item1, index) => (
                                                                                        <>
                                                                                            <h6 style={{ fontSize: '14px' }}>{index + 1}. {item1.options}</h6>
                                                                                        </>
                                                                                    ))}
